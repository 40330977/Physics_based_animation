...
int main()
{
	...
	glm::vec3 translator = glm::vec3(0.0f, 5.0f, 0.0f);
	...
	//std::vector<Hooke> HookeHolder(particlenum);
	for (unsigned int z1 = 1; z1 < particlenum - 1; z1++) {
		//HookeHolder[z1] = Hooke(&ParticleHolder[z1], &ParticleHolder[z1 + 1], ks, kd, rest);
