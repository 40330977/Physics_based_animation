...
Mesh::Mesh(MeshType type)
{
	...
	/*bool compare (Vertex i, Vertex j) {
		return(i == j);
	}*/

	//generate unique vertex vector (no duplicates)
	km_vertices = std::vector<Vertex>(std::begin(vertices), std::end(vertices));
	/*for each(Vertex k in km_vertices) {
		std::cout << glm::to_string(k.getCoord()) << std::endl;
	}*/
	//for (int i = 0; i < m_numIndices; i++)
	//	std::cout << glm::to_string(vertices[i].getCoord()) << std::endl << glm::to_string(km_vertices[i].getCoord()) << std::endl << std::endl;

	//m_vertices.resize(km_vertices.size());
	//std::unique(std::sort(m_vertices));
	//std::unique_copy
	...
	//for (auto it = std::begin(vertices); it != std::end(vertices); ++it) {
		/*for (auto it = 0; it != km_vertices.size(); ++it) {
			for (auto vert = 1; vert != km_vertices.size(); ++vert) {
				//m_vertices.push_back(std::unique_copy(km_vertices.begin(), km_vertices.end(), compare));
			//int vert = 1;
			if (&km_vertices[it] != &km_vertices[vert] ) {
					m_vertices.push_back(km_vertices[it]);
					//vert++;
					//std::cout <<glm::to_string(km_vertices[it].getCoord()) << std::endl;
					
				}
				//else { m_vertices.push_back(km_vertices[vert]); }
			}
			std::cout << glm::to_string(km_vertices[it].getCoord()) << std::endl;
		}*/

	/*for (auto i = km_vertices.begin(); i != km_vertices.end(); i++) {
		bool isunique = true;
		for (auto j = m_vertices.begin(); j != m_vertices.end(); j++) {
			...
			//std::cout <<glm::to_string(m_vertices.getCoord()) << std::endl;
		}
	}*/
	m_vertices.push_back(vertices[0]);

	//for (int i = 1; i < m_numIndices; i++) {
		for (int i = 1; i < 36; i++) {
		bool isUnique = true;
		for each (Vertex j in m_vertices) {
			if (vertices[i].getCoord() == j.getCoord()) {
				isUnique = false;
				break;
			}
		}
	...
	/*for each(Vertex k in m_vertices) {
		std::cout << glm::to_string(k.getCoord()) << std::endl;
	}*/
		...
		//std::qsort(vertices, km_vertices.size(), sizeof(vertices), compare);
...
void Mesh::setRotate(const glm::mat4 &mat) {
	m_rotate = mat;
}
